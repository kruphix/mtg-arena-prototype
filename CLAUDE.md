...
- `types.ts` — all core types: `Card`/`CardDefinition` (`text` is rules text; `flavor` is an optional separate field for flavor-only blurbs, rendered in italics — don't overload `text` with flavor), `Permanent` (a card instance on the battlefield with runtime state — tapped, summoning sickness, damage, temp buffs), `PlayerState`, `GameState`, `Phase`, `GameAction`.
...
The top-level layout in `App.tsx` is two columns: `.app__main` (both boards, both hands, both player panels, `CombatOverlay`) and a fixed-width `.app__sidebar` holding a vertical `PhaseBar` and the turn-history log. `PhaseBar`'s advance button reads "End Turn" during the `end` phase and "Next Phase" otherwise.

The active player's hand is always rendered at the bottom (face up); the opponent's hand is rendered face-down at the top (`Hand`'s `faceDown` prop, rendering blank `card--back` divs) — this flips each turn since it's hotseat play on one screen, not tied to `players[0]`/`players[1]`. `Board` takes a `landsFirst` prop, set for the opponent's board only, so lands render above non-lands there (mirrored) instead of below (as on the active player's board).

Battlefield cards (both players) and the active player's hand render at a wider size via `CardView`'s `wide` prop (`card--wide` class, 144px vs. the base 120px `.card`) — passed unconditionally from `Board` and from `Hand`'s face-up branch. The face-down opponent-hand branch renders `card--back` divs directly rather than `CardView`, so it's unaffected and stays at the smaller size; don't add `wide` there.

Blocker assignment is order-dependent by design: the defender must click an attacker first, then a blocker (`pendingBlockAttackerId` in `App.tsx`). `eligibleBlockerIds` returns an empty set until an attacker is pending — don't relax that check, it previously allowed blockers to be clicked (silently, with no effect) before an attacker was selected, which read as "blocking doesn't work." `eligibleBlockerIds` also filters to `type === 'creature'` (lands/spells on the battlefield shouldn't be selectable blockers) and excludes creatures already assigned to a *different* attacker, but keeps a creature assigned to the *currently pending* attacker selectable, so clicking it again toggles the assignment off — don't reintroduce a version that makes assigned blockers unclickable, that regresses "can't unselect a blocker."

### Card art (`public/cards/`)

Each card has an art file at `public/cards/<defId>.<ext>`, resolved through `CARD_ART` (`src/ui/cardArt.ts`) and rendered by `CardView` as `<img src={CARD_ART[def.id]}>`. `cardArt.ts` is generated, not hand-edited — it maps each card id to whichever art file currently exists for it.

There are two generators, both run via `node --experimental-strip-types scripts/<name>.mjs` since they import `CARDS` straight from `cards.ts`:

- `scripts/generate-card-art.mjs` — writes a placeholder SVG per card (colored rectangle by color, shape by type: diamond/land, circle/creature, star/spell). Re-run after adding cards so new ones get a placeholder.
- `scripts/import-card-art.mjs` — imports hand-generated art. Drop an image (png/jpg/jpeg/webp) per card into `art-source/` (gitignored staging dir, created on first run), named after the card id or name — e.g. `emberWhelp.png` or `Ember Whelp.jpg` both match the `emberWhelp` card — then re-run the script. Matched files are downscaled/cropped to `ART_SIZE` (currently 360x225 — 2.5x the `.card--wide`/`.card__art` box in `index.css`, for hidpi headroom; keep it in sync if those CSS dimensions change) and re-encoded as compressed `.webp` via `sharp` regardless of source resolution/aspect ratio, written to `public/cards/<id>.webp`, with any stale art in other formats for that id (including the old placeholder) deleted and `cardArt.ts` regenerated. Cards with no matching file keep their SVG placeholder.
