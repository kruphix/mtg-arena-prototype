...
Each card gets a small placeholder image at `public/cards/<defId>.svg`, referenced by `CardView` as `<img src={`/cards/${def.id}.svg`}>`. There's no database or dynamic lookup — the deck is a fixed static registry, so a static file per id is enough. Placeholders are generated by `scripts/generate-card-art.mjs` (run via `node --experimental-strip-types scripts/generate-card-art.mjs`, since it imports `CARDS` straight from `cards.ts`): a colored rectangle (flame/tide/colorless, matching the CSS theme vars) with a shape by type (diamond/land, circle/creature, star/spell). Re-run it after adding cards; replace individual files with real art later by keeping the same filename.
